...
bool AAOctreeVolume::IsValidDestLocation(const FVector& destination, const TArray<TEnumAsByte<EObjectTypeQuery>>& object_types, UClass* actor_class_filter)
{
	TArray<AActor*> outActors;
	bool traversable = !(UKismetSystemLibrary::BoxOverlapActors(GWorld, destination, FVector(GetDivisionSize() / 2.0f), object_types, actor_class_filter, TArray<AActor*>(), outActors));

	return traversable;
}

void AAOctreeVolume::OnConstruction(const FTransform& Transform)
{
...
void AAOctreeVolume::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	// Delete the nodes
	delete [] Nodes;
	Nodes = nullptr;
...
// Called every frame
void AAOctreeVolume::Tick(float DeltaTime)
{
...
FIntVector AAOctreeVolume::ConvertLocationToCoordinates(const FVector& location)
{
...
FVector AAOctreeVolume::ConvertCoordinatesToLocation(const FIntVector& coordinates)
{
...
void AAOctreeVolume::CreateLine(const FVector& start, const FVector& end, const FVector& normal, TArray<FVector>& vertices, TArray<int32>& triangles)
{
...
bool AAOctreeVolume::AreCoordinatesValid(const FIntVector& coordinates) const
{
...
void AAOctreeVolume::ClampCoordinates(FIntVector& coordinates) const
{
...
NavNode* AAOctreeVolume::GetNode(FIntVector coordinates) const
{
	ClampCoordinates(coordinates);
